...
subroutine rho_fine(ilevel,icount)
  ...
  real(dp)::d0,m_refine_loc,dx_min,vol_min,mstar,nISM,nCOM
  ...
  if(m_refine(ilevel)>-1.0d0)then
     ...
     do ind=1,twotondim
        ...
        if(hydro)then
           do i=1,active(ilevel)%ngrid
              phi(active(ilevel)%igrid(i)+iskip)=rho(active(ilevel)%igrid(i)+iskip)/d_scale
           end do
        endif
     end do
  endif
  ...
  if(m_refine(ilevel)>-1.0d0)then
     ...
     m_refine_loc=m_refine(ilevel)

     ! Prevent from refining too much relative
     ! to available star mass resolution
     if(star)then
        ! Finest cell size
        ...
        d0   = nISM/scale_nH
        ! Star particle mass
        mstar=MAX(del_star*omega_b*rhoc*X/mH,n_star)/(scale_nH*aexp**3)*vol_min/(1.0+eta_sn*(1d0+f_w))
        if(d0*(dx_loc/2.0)**ndim.lt.0.99*mstar)m_refine_loc=1d10
     endif
     
     do ind=1,twotondim
        iskip=ncoarse+(ind-1)*ngridmax
        do i=1,active(ilevel)%ngrid
           if(phi(active(ilevel)%igrid(i)+iskip)>=m_refine_loc)then
