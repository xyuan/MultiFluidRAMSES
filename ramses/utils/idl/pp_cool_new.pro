...
function cmp_cool,cool,nH,T2,ss=ss
...
if keyword_set(ss)then begin
   facH=facH+nH/1d-2/alog(10.) < 0.99*cool.n(n1-1)
endif
;facH=alog10(nH)
ix=facH-min(cool.n)
ix=ix*dlog_nH
...
function cmp_heat,cool,nH,T2,ss=ss
...
function cmp_heat_com,cool,nH,T2,ss=ss
...
fa=cool.heat_com(ix,iy)*w1H+cool.heat_com(ixp1,iy)*w2H
fb=cool.heat_com(ix,iyp1)*w1H+cool.heat_com(ixp1,iyp1)*w2H
fprimea=cool.heat_com_prime(ix,iy)*w1H+cool.heat_com_prime(ixp1,iy)*w2H
fprimeb=cool.heat_com_prime(ix,iyp1)*w1H+cool.heat_com_prime(ixp1,iyp1)*w2H
alpha=fprimea
beta=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
gamma=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
cmp_heat_com=10d0^(fa+alpha*yy+beta*yy2+gamma*yy3)

return,cmp_heat_com

end
function cmp_cool_com,cool,nH,T2,ss=ss
...
fa=cool.cool_com(ix,iy)*w1H+cool.cool_com(ixp1,iy)*w2H
fb=cool.cool_com(ix,iyp1)*w1H+cool.cool_com(ixp1,iyp1)*w2H
fprimea=cool.cool_com_prime(ix,iy)*w1H+cool.cool_com_prime(ixp1,iy)*w2H
fprimeb=cool.cool_com_prime(ix,iyp1)*w1H+cool.cool_com_prime(ixp1,iyp1)*w2H
alpha=fprimea
beta=3d0*(fb-fa)/h2-(2d0*fprimea+fprimeb)/h
gamma=(fprimea+fprimeb)/h2-2d0*(fb-fa)/h3
cmp_cool_com=10d0^(fa+alpha*yy+beta*yy2+gamma*yy3)

return,cmp_cool_com

end

function cmp_metal,cool,nH,T2,ss=ss
...
pro pp_cool,cin,z=z,compton=compton,ss=ss
...
if keyword_set(compton)then begin

   for j=0,n1-1 do begin
      nn=nH(j)

      heat=cmp_heat_com(cin,nn,T2)
      cool=cmp_cool_com(cin,nn,T2)
   ...
   for j=0,n1-1 do begin
      nn=nH(j)
      cool=cmp_cool(cin,nn,T2,ss=ss)
      heat=cmp_heat(cin,nn,T2,ss=ss)
      metal=cmp_metal(cin,nn,T2,ss=ss)
...
hhh=alog10(net)
mycontour,hhh,alog10(nH),alog10(t2),ncont=50,max=-20,min=-30.,xtitle='!17log n!dH!n (cm!u-3!n)',ytitle='!17log T (K)',title='!17Net cooling rate (erg cm!u3!n)'
