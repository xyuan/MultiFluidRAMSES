...
subroutine multigrid_fine(ilevel)
   ...
   real(dp) :: res_norm2, i_res_norm2, i_res_norm2_tot, res_norm2_tot
   real(dp) :: err, last_err
   ...
   main_iteration_loop: do
      ...
      last_err = err
      err = dsqrt(res_norm2/i_res_norm2)
      ...
      if(err > last_err*SAFE_FACTOR .and. (.not. safe_mode(ilevel))) then
         if(myid==1)print *,'CAUTION: Switching to safe MG mode for level ',ilevel
...
recursive subroutine recursive_multigrid_coarse(ifinelevel, safe)
   ...
   real(dp) :: res_norm2, res_norm2_tot
