...
!              notify outputs on screen          !
! 2009/03/03 : physical time                     !
!        /06 : physical distance                 !
!================================================!
...
subroutine dump_all(force)
  ...
  character(LEN=128)::filename,filedir,filecmd
  integer::info

  if(force)then
    write(*,*)'Dumping all data and aborting.'
  else
  ...
  if(ndim>1)then
    ...
    filedir=TRIM(outdir)//'/output_'//TRIM(nchar)//'/'
    if(myid==1)then
      write(*,*)
      write(*,*)'Output #',ifout,' at t = ',t,' (= ',t_phys,')'
      write(*,*)
#ifdef NOSYSTEM
      call PXFMKDIR(TRIM(filedir),LEN(TRIM(filedir)),O'755',info)
#else
      filecmd='mkdir -p '//TRIM(filedir)//ACHAR(0)
      call system(filecmd)
#endif
    endif
#ifndef WITHOUTMPI
...
!      end if
    ...
    if(hydro)then
       ...
       if(myid==1)then
          ...
          call dump_shock_history(filename)
       endif
    end if
    if(do_emission)then
...
subroutine backup_amr(filename)
  ...
  write(ilun)cons,mass_tot_0,rho_tot
...
subroutine output_info(filename)
  ...
  write(ilun,'("time        =",E23.15)')t      * code%t/user%t
  write(ilun,'("time phys   =",E23.15)')t_phys * code%t/user%t
