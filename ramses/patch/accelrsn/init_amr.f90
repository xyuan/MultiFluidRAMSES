...
! 2010/07/01 : removed check on noutput value    !
!================================================!
...
subroutine init_amr
 ...
 bisection_or_ordering:if(ordering /= 'bisection') then ! use usual ordering machinery
    ...
    do i=0,ndomain-1
       bound_key(i)=order_all_min+dble(i)/dble(ndomain)* &
  ...
  if(nrestart>0)then
     ...
     fileloc=TRIM(outdir)//'/output_'//TRIM(nchar)//'/amr_'//TRIM(nchar)//'.out'
