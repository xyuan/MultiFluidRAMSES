...
! subroutine cmp_cpumap                          !
! subroutine cmp_dommap                          !
! subroutine cmp_ordering                        !
! subroutine cmp_minmaxorder                     !
! subroutine defrag                              !
!================================================!
! 2008/09/24 : version 3.0                       !
! 2008/10/10 : new ordering types                !
!================================================!
...
subroutine cmp_new_cpu_map
  ...
  integer,dimension(1:overload)::ncell_sub,npart_sub
  real(kind=8),dimension(1:ndomain)::cost_loc,cost_old,cost_new
  real(kind=8),dimension(0:ndomain)::bound_key_loc
...
function theta(r)
  ...
  implicit none
  real(dp)::theta
  real(dp),dimension(1:3)::r,d=(/1,1,1/)
  real(dp)::rr,rd,dd,cos
  ! computes the angle between directions r and d
  
  rd = d(1)*r(1) + d(2)*r(2) + d(3)*r(3)
  rr = r(1)**2 + r(2)**2 + r(3)**2
  dd = d(1)**2 + d(2)**2 + d(3)**2
  cos = rd/sqrt(rr*dd)
  if(cos<-1)cos=-1
  if(cos>+1)cos=+1
  theta = (180./acos(-1.)) * acos(rd/sqrt(rr*dd))
...
subroutine cmp_ordering(x,order,nn)
...
#endif
  real(dp),dimension(1:nvector,1:ndim)::x
  real(dp),dimension(1:nvector)::order
  !--------------------------------------------------------
  ...
  integer::temp,info
  real(dp)::scale,bscale,xx,yy,zz,xc,yc,zc,theta
  ...
  if(ordering=='radial')then
     ! Radial domain decomposition
     do i=1,nn
        order(i)=sqrt(x(i,1)**2+x(i,2)**2+x(i,3)**2)
     end do
  end if

  if(ordering=='theta')then
     do i=1,nn
       order(i) = theta((/x(i,1),x(i,2),x(i,3)/))
     enddo
  endif
...
subroutine cmp_minmaxorder(x,order_min,order_max,dx,nn)
  ...
  real(dp)::dx
  real(dp),dimension(1:nvector,1:ndim)::x
  real(dp),dimension(1:nvector)::order_min,order_max
  !-----------------------------------------------------
  ...
  real(dp)::theta,theta1,theta2,theta3,theta4,dxx,dxmin  
  real(dp)::dkey,scale,bscaleloc,bscale,xx,yy,zz,xc,yc,zc,dir(1:3)=(/1,1,1/),xd,dd
  integer::eps_x,eps_y,eps_z
  ...
  if(ordering=='radial')then
     ! radial domain decomposition
     do i=1,nn
        order_min(i)=sqrt(x(i,1)**2+x(i,2)**2+x(i,3)**2)-sqrt(3.)*dxx
        order_max(i)=sqrt(x(i,1)**2+x(i,2)**2+x(i,3)**2)+sqrt(3.)*dxx
     end do
  end if
  ...
  if(ordering=='theta')then
    do i=1,nn
      ...
      do eps_y=-1,+1
      do eps_z=-1,+1
        theta1 = theta((/x(i,1)+eps_x*dxx,x(i,2)+eps_y*dxx,x(i,3)+eps_z*dxx/))
        theta2 = theta((/x(i,1)+eps_x*dxx,x(i,2)+eps_y*dxx,x(i,3)+eps_z*dxx/))
        theta3 = theta((/x(i,1)+eps_x*dxx,x(i,2)+eps_y*dxx,x(i,3)+eps_z*dxx/))
        order_min(i) = min(order_min(i),theta1,theta2,theta3)
        order_max(i) = max(order_max(i),theta1,theta2,theta3)
      enddo
      enddo
      enddo
    enddo
...
subroutine defrag
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=xg(igrid,idim)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     xg(igrid,idim)=hilbert_key(igrid)
  end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=uold(iskip2+igrid,ivar)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     uold(iskip2+igrid,ivar)=hilbert_key(igrid)
  end do
  end do
  end do
  ...
  do ilevel=1,nlevelmax
     do ibound=1,nboundary+ncpu
        ...
        if(ncache>0)then
           igrid=istart
           do i=1,ncache
              hilbert_key(ngrid2+i)=f(iskip2+igrid,idim)
              igrid=next(igrid)
           end do
           ngrid2=ngrid2+ncache
        end if
  ...
  do igrid=1,igridmax
     f(iskip2+igrid,idim)=hilbert_key(igrid)
  end do
  end do
  end do
