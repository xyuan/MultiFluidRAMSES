...
! subroutine init_flow_fine                      !
! subroutine region_condinit                     !
!================================================!
! 2008/09/24 : version 3.0                       !
! 2008/10/10 : condinit progression status       !
!     /06/08 : variable gamma                    !
!================================================!
...
subroutine init_flow_fine(ilevel)
  ...
  real(dp)::dx,rr,xx1,xx2,xx3,dx_loc,scale,e_kin
  ...
  if(ok_file)then
     ...
     do ivar=1,nvar_in
        if(cosmo)then
           ...
           if(ivar==6)filename=TRIM(initfile(ilevel))//'/ic_fp'
#ifdef VAR_G
           if(ivar==VAR_G)filename=TRIM(initfile(ilevel))//'/ic_g'
#endif
        endif
     ...
     !---------------------------------------------------
     ! For cosmology runs: compute initial pressure
     !---------------------------------------------------
     if(cosmo)then
     ...
     do igrid=1,ncache,nvector
        ...
        do ind=1,twotondim
           ...
           do i=1,ngrid
              ...
              ! Momentum
              uold(ind_cell(i),2:ndim+1) = rr * uold(ind_cell(i),2:ndim+1)
              ! Total energy
              uold(ind_cell(i),ndim+2) = uold(ind_cell(i),ndim+2)/(gamma-one) &
                                       + e_kin(uold(ind_cell(i),1),uold(ind_cell(i),2:ndim+1),xx(i,1:ndim))
              ! Passive scalars
              if(nvar>ndim+2)uold(ind_cell(i),ndim+3:nvar) = uold(ind_cell(i),1)*uold(ind_cell(i),ndim+3:nvar)
           enddo
        end do
        ! End loop over cells
...
if(myid==1)write(*,'("   Applying initial conditions on level ",I2)', advance='no')ilevel
threshold=10.
     ! Loop over grids by vector sweeps
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        ...
        ! End loop over cells
if(myid==1.and.igrid/(1.+floor(ncache/(1.*nvector))*nvector)>=threshold/100.)then
write(*,'(" ",I3,"%")', advance='no')int(100*igrid/(1.+floor(ncache/(1.*nvector))*nvector))
