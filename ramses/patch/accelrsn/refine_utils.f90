...
! subroutine make_grid_coarse                    !
! subroutine refine_fine                         !
! subroutine make_grid_fine                      !
! subroutine kill_grid                           !
!================================================!
! 2008/09/24 : version 3.0                       !
! 2010/05/26 : added cell positions              !
!================================================!
...
subroutine refine_coarse
  ...
  do i=icoarse_min,icoarse_max
     ...
     if(flag1(ind)==0.and.son(ind)>0)then
        nkill=nkill+1
        ind_cell(1)=ind
        call kill_grid(ind_cell,1,1,ibound,boundary_region)
     end if
  end do
  end do
  end do
...
subroutine make_grid_fine(ind_grid,ind_cell,ind,ilevel,nn,ibound,boundary_region)
  ...
  if(.not.init .and. hydro .and. .not.balance)then
     ...
     ! Gather father variables
     do j=0,twondim
        do i=1,nn
#ifdef SOLVERmhd
           u1(i,j,1:nvar+3) =     uold(ind_fathers(i,j),1:nvar+3)  ! Hydro variables
#else
           u1(i,j,1:nvar  ) =     uold(ind_fathers(i,j),1:nvar  )  ! Hydro variables
#endif
           x1(i,j,1:ndim  ) = position(ind_fathers(i,j),1:ndim  )  ! Positions
           if(poisson)then
           g1(i,j,1:ndim  ) =        f(ind_fathers(i,j),1:ndim  )  ! Gravity acceleration
           end if
#ifdef SOLVERmhd
           ind1(i,j)=son(ind_fathers(i,j))  ! Son index
#endif
        end do
      end do
     ! Interpolate
#ifdef SOLVERmhd
     call interpol_hydro(u1,x1,g1,ind1,u2,x2,g2,nn)
#else
     call interpol_hydro(u1,x1,g1,u2,x2,g2,nn)
#endif
     ...
     do j=1,twotondim
        ...
        do i=1,nn
#ifdef SOLVERmhd
           uold    (iskip+ind_grid_son(i),1:nvar+3) = u2(i,j,1:nvar+3)
#else
           uold    (iskip+ind_grid_son(i),1:nvar  ) = u2(i,j,1:nvar  )
#endif
           position(iskip+ind_grid_son(i),1:ndim  ) = x2(i,j,1:ndim  )
           if(poisson)then
           f       (iskip+ind_grid_son(i),1:ndim  ) = g2(i,j,1:ndim  )
           end if
        end do
     end do
  end if
