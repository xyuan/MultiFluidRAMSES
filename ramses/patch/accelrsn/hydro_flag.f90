...
! HYDRO REFINEMENT                               !
!================================================!
! subroutine hydro_flag                          !
! subroutine jeans_length_refine                 !
!================================================!
! 2008/09/24 : version 3.0                       !
! 2009/06/01 : added error on ejecta fraction    !
! 2010/05/30 : added cell positions              !
!================================================!
...
subroutine hydro_flag(ilevel)
  ...
  real(dp),dimension(1:nvector,1:ndim),save::xxg,xxm,xxd
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do idim=1,ndim
           ! Gather hydro variables and positions
           do i=1,ngrid
               uug(i,1:nvar) =     uold(indn(i,2*idim-1),1:nvar)
               uum(i,1:nvar) =     uold(ind_cell(i     ),1:nvar)
               uud(i,1:nvar) =     uold(indn(i,2*idim  ),1:nvar)
               xxg(i,1:ndim) = position(indn(i,2*idim-1),1:ndim)
               xxm(i,1:ndim) = position(ind_cell(i     ),1:ndim)
               xxd(i,1:ndim) = position(indn(i,2*idim  ),1:ndim)
           end do
           call hydro_refine(uug,uum,uud,xxg,xxm,xxd,ok,ngrid)
        end do
