...
module amr_parameters
  ...
  integer,dimension(0:6)::show_fields=-1 ! List of regions to monitor
  ...
  integer::ifout=1            ! Start index for output files (=0 as automatic output at t=0)
  ...
  character(len=128)::outdir='.'         ! Output directory
  logical::monitor_grid=.false.   ! Monitor grid structure
  logical::monitor_hydro=.false.   ! Monitor SNR mass and energy
  ...
  real(dp)::f_ek   =1.0D0     ! Supernovae kinetic energy fraction (only 0 or 1)
  ...
  character(LEN=128),dimension(1:MAXLEVEL)::initfile=' '
