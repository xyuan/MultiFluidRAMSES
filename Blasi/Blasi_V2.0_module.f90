!===========================================================================================!
! Blasi's non-linear model for Diffusive Shock Acceleration                           v 2.0 !
!===========================================================================================!
! units:                                                                                    !
...
module Blasi
    ...
    type NDSA_results
        ...
        real*8,pointer::B(:)=>null()  ! total magnetic field    
        ! PARTICLES
        ! injection
        ...
        real*8,pointer::p_p(:)=>null()  ! momenta
        real*8,pointer::D_p(:)=>null()  ! diffusion coefficient
        real*8,pointer::f_p(:)=>null()  ! distribution of particles at the shock
        real*8,pointer::s_p(:)=>null()  ! distribution of particles escaping upstream
        ...
        real*8,pointer::p_e(:)=>null()  ! momenta
        real*8,pointer::f_e(:)=>null()  ! distribution of particles at the shock
    end type
    type(NDSA_results),pointer,save::OUT(:)=>null()
...
!===========================================================================================
    ...
    if(associated(OUT))then
      do i_sol=lbound(OUT,1),ubound(OUT,1)
        if(associated(OUT(i_sol)%x  ))deallocate(OUT(i_sol)%x  )
        if(associated(OUT(i_sol)%U  ))deallocate(OUT(i_sol)%U  )
        if(associated(OUT(i_sol)%P  ))deallocate(OUT(i_sol)%P  )
        if(associated(OUT(i_sol)%B  ))deallocate(OUT(i_sol)%B  )
        if(associated(OUT(i_sol)%p_p))deallocate(OUT(i_sol)%p_p)
        if(associated(OUT(i_sol)%D_p))deallocate(OUT(i_sol)%D_p)
        if(associated(OUT(i_sol)%f_p))deallocate(OUT(i_sol)%f_p)
        if(associated(OUT(i_sol)%s_p))deallocate(OUT(i_sol)%s_p)
        if(associated(OUT(i_sol)%p_e))deallocate(OUT(i_sol)%p_e)
        if(associated(OUT(i_sol)%f_e))deallocate(OUT(i_sol)%f_e)
      enddo
      deallocate(OUT)
    endif
...
!===========================================================================================
    ...
    do i_sol=1,n_sol
        ...
        OUT(i_sol)%U => reallocate(OUT(i_sol)%U,-1,n_nt)
...
!===========================================================================================
    ...
    if(i_max_new>=lbound(array,1).and.i_min_new<=ubound(array,1))then
