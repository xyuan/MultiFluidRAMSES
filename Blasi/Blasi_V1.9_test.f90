!===========================================================================================!
! Blasi's non-linear model for Diffusive Shock Acceleration                           v 1.9 !
!===========================================================================================!
...
!===========================================================================================!
! Gilles Ferrand (CEA/Irfu/SAp)          first version 07/04/2009, last revision 02/04/2010 !
!===========================================================================================!
...
program test
    ...
    real*8::Emax_e=-1   ! maximum energy of electrons [mp.c units]
    ...
    namelist /params/ Gth, u0, T0, M0, n0, mu_d, mu_P, Ma0, B0, zeta, D0, alpha, xi, pinj, eta, Emax_p, tmax_p, xmax_p, cut_p, &
                      Emax_e, cut_e, kappa, chi, i_sol, pres, verbose, escape
    ...
    escape = .true. ! to compute the distribution of escaping particles (slower)
    ...
    if(Ma0>0)then
        ...
        B0 = B0 * 1D-6 ! muG -> G
        write(*,*)'Ma0 = ',u0/(B0/sqrt(4*pi*mp*n0))
    endif
    write(*,*)
    ...
    do i_sol=1,n_sol
        ...
        call print_screen(i_sol,n_sol,Rsub(i_sol),Rtot(i_sol),T2(i_sol),prec(i_sol)%B(0),B2(i_sol),prec(i_sol)%P(0),&
                xi,eta_xi(i_sol),pinj_xi(i_sol),Pcr(i_sol),Wcr(i_sol),Gcr(i_sol),Fesc(i_sol),&
...
if(.false.)then
    ...
    contains
        ...
        subroutine print_screen(i_sol,n_sol,Rsub,Rtot,T2,B1,B2,P1,xi,eta_xi,pinj_xi,Pcr,Wcr,Gcr,Fesc,pinj_p,pmax_p,pinj_e,pmax_e)
            
            implicit none
            
            integer::i_sol,n_sol
            real*8::Rsub,Rtot,T2,B1,B2,P1,xi,eta_xi,pinj_xi,Pcr,Wcr,Gcr,Fesc,pinj_p,pmax_p,pinj_e,pmax_e
