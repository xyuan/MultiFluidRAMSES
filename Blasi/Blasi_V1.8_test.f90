!===========================================================================================!
! Blasi's non-linear model for Diffusive Shock Acceleration                           v 1.8 !
!===========================================================================================!
...
!===========================================================================================!
! Gilles Ferrand (CEA/Irfu/SAp)          first version 07/04/2009, last revision 27/03/2010 !
!===========================================================================================!
...
program test
    ...
    real*8::u0=5000       ! upstream velocity [km/s]   ! only one of the two
    ...
    namelist /params/ Gth, u0, T0, M0, n0, mu_d, mu_P, Ma0, B0, zeta, D0, alpha, xi, pinj, eta, Emax_p, tmax_p, xmax_p, cut_p, &
    ...
    if(u0>0.and.T0>0)then
        write(*,*)'set only u0 or T0'
        stop 1
    endif
    if(u0<=0)then
        u0 = sqrt((Gth*mu_P*kB)/(mu_d*mp)*M0**2*T0)
        write(*,*)'u0 = ',u0/1e5,' km/s'
    else
        u0 = u0 * 1D+5 ! km/s -> cm/s
    endif
    if(T0<=0)then
        T0 = (mu_d*mp)/(Gth*mu_P*kB)*(u0/M0)**2
        write(*,*)'T0 = ',T0,' K'
    endif
    n0 = n0 * mu_d
    if(Ma0>0)then
    ...
    write(*,*)'Blasi_DSA'
    write(*,*)'---------'
    call Blasi_DSA(Gth, M0, u0, T0, n0, B0, zeta, D0, alpha, &
    ...
    do i_sol=1,n_sol
        ...
        call print_screen(i_sol,n_sol,Rsub(i_sol),Rtot(i_sol),T2(i_sol),prec(i_sol)%B(0),B2(i_sol),prec(i_sol)%P(0),&
                xi,eta_xi(i_sol),pinj_xi(i_sol),Pcr(i_sol),Wcr(i_sol),Gcr(i_sol),Fesc(i_sol),spec_p(i_sol)%p,spec_e(i_sol)%p)
        ...
        write(*,*)
        if(associated(spec_p(i_sol)%f)) call print_file(outfile,i_sol,'fp'," p/mc                      p^4.f/mc",&
                      spec_p(i_sol)%p/mc    ,spec_p(i_sol)%f*spec_p(i_sol)%p**4/mc)
        if(associated(spec_e(i_sol)%f)) call print_file(outfile,i_sol,'fe'," p/mc                      p^4.f/mc",&
                      spec_e(i_sol)%p/mc    ,spec_e(i_sol)%f*spec_e(i_sol)%p**4/mc)
        if(associated(  prec(i_sol)%u)) call print_file(outfile,i_sol,'u'," x (pc)                      u (1e6 m/s)",&
                        prec(i_sol)%x/parsec,  prec(i_sol)%u/1e5                  )
        if(associated(  prec(i_sol)%P)) call print_file(outfile,i_sol,'P'," x (pc)                      P (erg/cm3)",&
                        prec(i_sol)%x/parsec,  prec(i_sol)%P                      )
        if(associated(  prec(i_sol)%B)) call print_file(outfile,i_sol,'B'," x (pc)                      B (1e-6 G)",&
    ...
    write(*,*)'---------------'
    call Blasi_backreact(Gth, M0, u0, T0, n0, B0, zeta, D0, alpha, &
    ...
    contains
        ...
        subroutine print_screen(i_sol,n_sol,Rsub,Rtot,T2,B1,B2,P1,xi,eta_xi,pinj_xi,Pcr,Wcr,Gcr,Fesc,p_p,p_e)
            
            implicit none
            
            integer::i_sol,n_sol
            real*8::Rsub,Rtot,T2,B1,B2,P1,xi,eta_xi,pinj_xi,Pcr,Wcr,Gcr,Fesc
