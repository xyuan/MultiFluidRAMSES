...
module Blasi
    ...
    type NDSA_params
        ...
        real*8,allocatable::f_p(:)  ! distribution
        real*8::Pcr0                ! pressure
        real*8::Ecr0                ! internal energy
        real*8::Gcr0                ! adiabatic index
        ! diffusion
    ...
    integer,parameter::n_sol_max=1      ! maximum number of non-linear solutions to be searched (set to 1 to only get the least modified one)
...
!===========================================================================================
    ...
    do i_sol=1,n_sol
        ...
        ! compute the relative pressure and adiabatic index of non-thermal protons at the shock
        
        call compute_cr_fluid(OUT(i_sol)%p_p(0:),OUT(i_sol)%f_p(0:),&
                              OUT(i_sol)%Pcr,OUT(i_sol)%Ecr,OUT(i_sol)%Gcr)
        ...
        ! compute the escaping flux of non-thermal protons
        
        call compute_escape_flux(OUT(i_sol)%p_p(0:),OUT(i_sol)%f_p(0:),OUT(i_sol)%s_p(0:),OUT(i_sol)%Rtot,&
                                 OUT(i_sol)%Fesc)
        ...
        if(delta>0.05)then
            ...
            call error("compute_escape_flux", message, delta>0.5.and.(.not.IN%linear))
        endif
        ...
        if(IN%cut_p>0) call add_cutoff(-1d0, IN%cut_p, IN%pres, OUT(i_sol)%p_p, OUT(i_sol)%f_p)
        if(IN%cut_e>0) call add_cutoff(-1d0, IN%cut_e, IN%pres, OUT(i_sol)%p_e, OUT(i_sol)%f_e)
...
!===========================================================================================
 subroutine add_cutoff(pcut_over_pmax, cut, pres, p, f)
!===========================================================================================
...
!===========================================================================================
    ...
    ! inputs
    real*8::pcut_over_pmax ! cut-off momentum, in terms of the maximum momentum
...
!===========================================================================================
 subroutine compute_cr_fluid(p,f,Pcr,Ecr,Gcr)
!===========================================================================================
! computes the pressure, internal energy and adiabatic index of particles
!===========================================================================================
    
    ! inputs
    real*8::p(0:)  ! momenta
    real*8::f(0:)  ! distribution of particles
    ! output
    real*8::Pcr    ! pressure
    real*8::Ecr    ! internal energy
    real*8::Gcr    ! adiabatic index
    ...
    ! adiabatic index = 1 + P/E
    
    Gcr = 1 + Pcr/Ecr
    
    if( (Gcr<4/3.*(1-tol_min)) .or. (Gcr>IN%Gth*(1+tol_min)) )then
        write(message,'("adiabatic index of the particles = ",F6.3)')Gcr
...
!===========================================================================================
 subroutine compute_escape_flux(p,f,s,Rtot,Fesc)
!===========================================================================================
! computes the escaping flux of particles
!===========================================================================================
    
    ! inputs
    real*8::p(0:)  ! momenta
    real*8::f(0:)  ! distribution of particles at the shock
    real*8::s(0:)  ! distribution of particles escaping upstream
    real*8::Rtot   ! total compression factor
    ! output
    real*8::Fesc   ! escaping flux of particles
    
    ! locals
    real*8::x_left,x_right
    integer::imax,i
    
    if(IN%verbose>=2) write(*,*)"  compute_escape_flux()"
    ...
    if(IN%escape)then
        ...
        Fesc = (4.*pi/3.) * (1-1/Rtot) * p(imax)**3 * f(imax) * (sqrt(1+(p(imax)/mc)**2)-1)*mc2
    endif
...
!===========================================================================================
    ...
    if(IN%linear)then
        ...
        format = '("   accel_protons(): linear solution : r = ",F10.6,",",F10.6,",",F10.6," -> U_max = ",F11.6)'
        if(IN%verbose>=2) write(*,format)OUT(i_sol)%Rprec,OUT(i_sol)%Rsub,OUT(i_sol)%Rtot,OUT(i_sol)%u(ubound(OUT(i_sol)%u,1))
        n_sol = 1
    else
        n_sol = find_NL_solutions()
    endif
    ...
    do i_sol=1,n_sol
        ...
        call reallocate(OUT(i_sol)%s_p,lbound(OUT(i_sol)%p_p,1),ubound(OUT(i_sol)%p_p,1))
        OUT(i_sol)%s_p(:0) = 0.
        OUT(i_sol)%s_p(0:) = OUT(i_sol)%s_p(0:) * OUT(i_sol)%f_p(0:)
        
        ! reconstruct precursor (x, u, P, B)
        ...
        do i=-1,n_nt
            if(IN%alpha<0)then ! Bohm: D(p) = D0/B.p^2/sqrt(1+p^2)
                OUT(i_sol)%D_p(i) = (OUT(i_sol)%p_p(i)/mc)**2/sqrt(1+(OUT(i_sol)%p_p(i)/mc)**2)
            else ! power-law: D(p) = D0/B.p^a
                OUT(i_sol)%D_p(i) = (OUT(i_sol)%p_p(i)/mc)**IN%alpha
            endif
        enddo
        ...
        OUT(i_sol)%u(0:) = OUT(i_sol)%U(0:) * IN%u0
...
!===========================================================================================
 function find_NL_solutions()
!===========================================================================================
! find all non-linear solutions by browsing all possible compression ratios
!===========================================================================================
    
    implicit none
    
    ! outputs
    integer::find_NL_solutions          ! number of solutions
    ! locals
    integer::n_sol                      ! number of solutions
    ...
    do while(OUT(i_sol)%Rsub>1.1 .and. n_sol<n_sol_max) ! Rsub > 1 to avoid numerical difficulties
        OUT(i_sol)%Rprec = OUT(i_sol)%Rprec + dR
        call solve_coupled_system()
        U_max_old = Umax
        Umax = OUT(i_sol)%u(ubound(OUT(i_sol)%u,1))
        if((U_max_old-1)*(Umax-1)<=0) then
            n_sol = n_sol + 1
    ...
    find_NL_solutions = n_sol
    
 end function find_NL_solutions
...
!===========================================================================================
    ...
    allocate(OUT(i_sol)%s_p(0:ubound(OUT(i_sol)%p_p,1)))
    OUT(i_sol)%s_p(:) = 0
    
    if(IN%escape)then
        ! diffusion coefficient
        allocate(K(0:ubound(OUT(i_sol)%p_p,1)))
        do i=0,ubound(OUT(i_sol)%p_p,1)
            if(IN%alpha<0)then ! Bohm: D(p) = D0/B.p^2/sqrt(1+p^2)
                K(i) = (OUT(i_sol)%p_p(i)/mc)**2/sqrt(1+(OUT(i_sol)%p_p(i)/mc)**2)
            else ! power-law: D(p) = D0/B.p^a
                K(i) = (OUT(i_sol)%p_p(i)/mc)**IN%alpha
            endif
        enddo
    endif
    ...
    if(IN%escape)call compute_escape(K)
    call compute_f_from_U()
    norm = tol_min
    iter = 1
    do while(norm>=tol_min.and.iter<iter_max.and.(.not.IN%linear))
        U_old(:) = OUT(i_sol)%U(:)
...
!===========================================================================================
    ...
    if(IN%eta>=0)then
        xi_left  = sqrt(3/2.)
        xi_right = 9.
        eta_xi = 4./(3.*sqrt(pi)) * (OUT(i_sol)%Rsub-1.) * xi_right**3*exp(-xi_right**2)
        if(IN%eta<eta_xi)then
            OUT(i_sol)%xi_eta = xi_right
        else
            delta = 2*delta_max
            iter = 0
...
!===========================================================================================
    ...
    X(:) = K(:) / OUT(i_sol)%U(:)
    psi(0) = 0
    do j=1,ubound(OUT(i_sol)%U,1)
        psi(j) = psi(j-1) + OUT(i_sol)%U(j) * (X(j)-X(j-1))
    enddo
    do i=0,ubound(OUT(i_sol)%U,1)
        W(i) = 0
        x_left = exp(psi(0)/K(i))/K(i)
        do j=1,OUT(i_sol)%imax_p
            x_right = exp(psi(j)/K(i))/K(i)
            W(i) = W(i) + 0.5*(x_left+x_right)*(X(j)-X(j-1))
            x_left = x_right
        enddo
        OUT(i_sol)%s_p(i) = 1 / W(i)
    enddo
...
!===========================================================================================
    ...
    ! locals
    real*8,allocatable::f_inj(:)  ! acceleration of freshly injected particles
    real*8,allocatable::f_seed(:) ! re-acceleration of upstream particles
    real*8,allocatable::f0(:)     ! seed distribution, re-binned to the same grid as f(p)
    real*8,allocatable::temp(:,:)
    real*8::x_left,x_right
    integer::i,j
    
    allocate(f_inj (0:ubound(OUT(i_sol)%p_p,1)))
    allocate(f_seed(0:ubound(OUT(i_sol)%p_p,1)))
    f_inj  = 0
    f_seed = 0
    
    ! re-acceleration of seed particles
    
    if(allocated(IN%f_p))then
        
        allocate(temp(0:ubound(OUT(i_sol)%p_p,1),0:ubound(OUT(i_sol)%p_p,1)))
        allocate(f0  (0:ubound(OUT(i_sol)%p_p,1)))
        
        do j = 0,ubound(OUT(i_sol)%p_p,1)
            if((OUT(i_sol)%p_p(j) >= IN%p_p(lbound(IN%p_p,1))).and.&
               (OUT(i_sol)%p_p(j) <= IN%p_p(ubound(IN%p_p,1))) )then 
                f0(j) = exp(DIVDIF(log(IN%f_p(0:)),log(IN%p_p(0:)),size(IN%p_p(0:)),log(OUT(i_sol)%p_p(j)),3))
            else 
                f0(j) = 0
            endif
            i = j
            temp(j,i) = 0.
            x_left = (OUT(i_sol)%U(i))/(OUT(i_sol)%U(i)-1/OUT(i_sol)%Rtot)
            do i = j+1,ubound(OUT(i_sol)%p_p,1)
                x_right = (OUT(i_sol)%U(i))/(OUT(i_sol)%U(i)-1/OUT(i_sol)%Rtot)
                temp(j,i) = temp(j,i-1) &
                          + 0.5*(x_left+x_right)*log(OUT(i_sol)%p_p(i)/OUT(i_sol)%p_p(i-1))
                x_left = x_right
            enddo
        enddo
        ...
        do i = 0,ubound(OUT(i_sol)%p_p,1)
            f_seed(i) = 0.
            j = 0
            x_left = f0(j) * exp(-3*temp(j,i))
            do j = 0,i
                x_right = f0(j) * exp(-3*temp(j,i))
                f_seed(i) = f_seed(i) &
                          + 0.5*(x_left+x_right)*log(OUT(i_sol)%p_p(j)/OUT(i_sol)%p_p(j-1))
                x_left = x_right
            enddo
        enddo
    ...
    ! acceleration of freshly-injected particles
    
    if(OUT(i_sol)%eta_p>0)then
        
        i = 0
        f_inj(i) = 0.
        x_left = (OUT(i_sol)%U(i)+OUT(i_sol)%s_p(i))/(OUT(i_sol)%U(i)-1/OUT(i_sol)%Rtot)
        do i = 1,ubound(OUT(i_sol)%p_p,1)
            x_right = (OUT(i_sol)%U(i)+OUT(i_sol)%s_p(i))/(OUT(i_sol)%U(i)-1/OUT(i_sol)%Rtot)
            f_inj(i) = f_inj(i-1) &
                     + 0.5*(x_left+x_right)*log(OUT(i_sol)%p_p(i)/OUT(i_sol)%p_p(i-1))
            x_left = x_right
        enddo
        
        f_inj(0:) = OUT(i_sol)%eta_p/(4*pi*OUT(i_sol)%pinj_p**3) * exp(-3*f_inj(0:))
        
    endif
    
    ! sum
    
    OUT(i_sol)%f_p(0:) = 3/(OUT(i_sol)%U(0:)-1/OUT(i_sol)%Rtot) * (f_inj(0:) + f_seed(0:))
    
!do i=0,ubound(OUT(i_sol)%p_p,1)
!write(*,*)'i = ',i,&
!': p = ',OUT(i_sol)%p_p(i)                     /mc,&
!' : ('  ,OUT(i_sol)%p_p(i)**4*            f0(i)/mc,&
!' -> '  ,OUT(i_sol)%p_p(i)**4*        f_seed(i)/mc,&
!' + '   ,OUT(i_sol)%p_p(i)**4*        f_inj (i)/mc,&
!' ) * ' ,3/(OUT(i_sol)%U(i)-1/OUT(i_sol)%Rtot),&
!' = '   ,OUT(i_sol)%p_p(i)**4*OUT(i_sol)%f_p(i)/mc
!enddo
    if(minval(OUT(i_sol)%f_p(0:))<0) call error("compute_f_from_U","negative density",.true.)
    
    deallocate(f_inj,f_seed)
...
!write(*,*)
!write(*,*)IN%Gcr0/IN%Gth,IN%Pcr0/IN%P0
    OUT(i_sol)%U(0) = 1/OUT(i_sol)%Rprec
i = -1
!write(*,*)i+1,OUT(i_sol)%U(i+1),OUT(i_sol)%U(i+1)*OUT(i_sol)%Rprec,OUT(i_sol)%U(i+1)*OUT(i_sol)%Rtot
    
    do i = 0,ubound(OUT(i_sol)%p_p,1)-1
        
        dlnp = log(OUT(i_sol)%p_p(i+1)/OUT(i_sol)%p_p(i))
        
        ! evaluate quantities after half the step, using current values
        U_ref = OUT(i_sol)%U(i)
        p_ref = OUT(i_sol)%p_p(i) / mc
        f_ref = OUT(i_sol)%f_p(i)
        source = 4.*pi*mc**3/(3.*(IN%u0/c)**2) * p_ref**5/sqrt(1.+p_ref**2) * f_ref &
               / (1. - U_ref**(-IN%Gcr0-1) / IN%Ms0**2 * (IN%Gcr0/IN%Gth)*(IN%Pcr0/IN%P0) &
                     - U_ref**(-IN%Gth -1) / IN%Ms0**2 &
                     - U_ref**(-IN%Gth -1) / IN%Ma0    * IN%zeta*(IN%Gth-1) &
                     - (1-IN%zeta)*(U_ref**2+3)/(8*IN%Ma0*U_ref**(5/2.)) &
                  )
!write(*,*)source*0.5*dlnp
        ! evaluate quantities after the full step, using half-step values
        U_ref = OUT(i_sol)%U(i) + source*0.5*dlnp
        p_ref = exp(log(OUT(i_sol)%p_p(i))+0.5*dlnp) / mc
        f_ref = 0.5*(OUT(i_sol)%f_p(i)+OUT(i_sol)%f_p(i+1))
        source = 4.*pi*mc**3/(3.*(IN%u0/c)**2) * p_ref**5/sqrt(1.+p_ref**2) * f_ref &
               / (1. - U_ref**(-IN%Gcr0-1) / IN%Ms0**2 * (IN%Gcr0/IN%Gth)*(IN%Pcr0/IN%P0) &
                     - U_ref**(-IN%Gth -1) / IN%Ms0**2 &
                     - U_ref**(-IN%Gth -1) / IN%Ma0    * IN%zeta*(IN%Gth-1) &
...
!write(*,*)source*dlnp
        ...
        OUT(i_sol)%U(i+1) = OUT(i_sol)%U(i) + source*dlnp
        
!write(*,*)i+1,OUT(i_sol)%U(i+1),OUT(i_sol)%U(i+1)*OUT(i_sol)%Rprec,OUT(i_sol)%U(i+1)*OUT(i_sol)%Rtot
        if(OUT(i_sol)%U(i+1)<1/OUT(i_sol)%Rprec)then
            call error("compute_U_from_f","inconsistent velocities",.true.)
        endif
...
!===========================================================================================
    ...
    if(IN%Emax_e>0)then ! fixed
        ...
        do while(delta>delta_min.and.iter<iter_max)
            i = ubound(OUT(i_sol)%p_p,1) - 1
            do while(OUT(i_sol)%p_p(i)>pmax_e.and.i>0)
                i = i-1
            enddo
            frac = (pmax_e-OUT(i_sol)%p_p(i))/(OUT(i_sol)%p_p(i+1)-OUT(i_sol)%p_p(i))
            Up_max = (OUT(i_sol)%u(i)+frac*(OUT(i_sol)%u(i+1)-OUT(i_sol)%u(i))) / u0
    ...
    do i = 0,n_e
        lnp_e(i) = log(pmin_e) + (log(OUT(i_sol)%pmax_e) - log(pmin_e)) * (float(i) / float(n_e))
    enddo
    ...
    do while(lnp_e(i)>=lnp_e_inj_p.and.i>=0)
        if(lnp_e(i)>=log(100*mc).or.i>=n_e) then
            ! 1/ shift protons spectrum
            lnf_e(i) = DIVDIF(lnf_p,lnp_p,size(lnp_p),lnp_e(i),3) + log(IN%kappa)
        else
    ...
    i = n_e
    MaxBol = OUT(i_sol)%n2/(pi**1.5*OUT(i_sol)%pth_e**3) * exp(-(exp(lnp_e(i))/OUT(i_sol)%pth_e)**2)
    do while(MaxBol<exp(lnf_e(i)).and.i>0)
        i = i-1
        MaxBol = OUT(i_sol)%n2/(pi**1.5*OUT(i_sol)%pth_e**3) * exp(-(exp(lnp_e(i))/OUT(i_sol)%pth_e)**2)
    enddo
    ...
    do i = lbound(OUT(i_sol)%p_e,1),ubound(OUT(i_sol)%p_e,1),+1
        OUT(i_sol)%f_e(i) = exp(DIVDIF(lnf_e,lnp_e,n_e,log(OUT(i_sol)%p_e(i)),3))
    enddo
    OUT(i_sol)%imax_e = ubound(OUT(i_sol)%p_e,1)
    
    ! add Maxwellian distribution
    do i = 0,lbound(OUT(i_sol)%p_e,1),-1
        OUT(i_sol)%f_e(i) = OUT(i_sol)%n2/(pi**1.5*OUT(i_sol)%pth_e**3) * exp(-(OUT(i_sol)%p_e(i)/OUT(i_sol)%pth_e)**2)
    enddo
...
!===========================================================================================
    ...
    write(*,*)
    if(abort) then
        write(*,*)"ERROR in routine ",routine,"() of module Blasi: ",trim(message)
    else
        write(*,*)"WARNING in routine ",routine,"() of module Blasi: ",trim(message)
    endif
    write(*,*)
    if(abort) stop 1
