!===========================================================================================!
! Blasi's non-linear model for Diffusive Shock Acceleration                v 1.8 (27/03/10) !
!===========================================================================================!
! units:                                                                                    !
...
module blasi_module
    ...
    real*8,parameter::delta_min=1e-6    ! numerical tolerance for numerical checks: requested precision
    real*8,parameter::delta_max=1e-3    ! numerical tolerance for numerical checks: accepted  precision
...
!===========================================================================================
    ...
    call Blasi_DSA(Gth, Ms0, u0, T0, n0, B0, zeta, D0, alpha, &
                   ...
                   Rsub, Rtot, T2, B2, prec, pinj_xi, eta_xi, spec_p, spec_e, Pcr, Wcr, Gcr, Fesc, n_sol)
...
!===========================================================================================
    ...
    ! locals
    integer::i_sol,i          ! which solution
    ...
    do i_sol=1,n_sol
        ...
        call compute_cr_fluid(spec_p(i_sol)%p(0:), spec_p(i_sol)%f(0:), Rtot(i_sol), &
...
!===========================================================================================
 subroutine compute_cr_fluid(p, f, Rtot, &
                             Pcr, Ecr, Fesc)
!===========================================================================================
...
!===========================================================================================
 subroutine accel_protons(Gth, u0, T0, Ms0, n0, B0, zeta, D0, alpha, xi, pinj0, eta0, Emax, tmax, xmax, i_sol, pres, &
                          Rsub, Rtot, P2, T2, Pw2, prec, pinj_xi, eta_xi, spec_p, n_sol)
!===========================================================================================
...
!===========================================================================================
    ...
    real*8::Rsub_i,Rtot_i,P2_i,T2_i,Pw2_i,pinj_xi_i,eta_xi_i  ! outputs for current solution
    real*8,pointer::p_i(:),f_i(:),U_i(:),K_i(:),B_i(:)        ! 
    ...
    call solve_coupled_system(Gth, u0, P0, T0, Ms0, Ma0, zeta, Rprec_i, &
                              ...
                              Rsub_i, Rtot_i, P2_i, T2_i, Pw2_i, pinj_xi_i, eta_xi_i, p_i, f_i, U_i)
    ...
    do i_sol=1,n_sol
        ...
        allocate(K_i(0:ubound(p_i,1)))
        allocate(B_i(0:ubound(p_i,1)))
        ...
        allocate(prec(i_sol)%x(-1:ubound(p_i,1)))
        allocate(prec(i_sol)%u(-1:ubound(p_i,1)))
        allocate(prec(i_sol)%P(-1:ubound(p_i,1)))
        allocate(prec(i_sol)%B(-1:ubound(p_i,1)))
        prec(i_sol)%x(-1) = 0.
        prec(i_sol)%u(-1) = u0 / Rtot_i
        prec(i_sol)%P(-1) = P2_i
        prec(i_sol)%B(-1) = sqrt(8*pi*Pw2_i+B0**2)
        prec(i_sol)%x(0:ubound(p_i,1)) = (K_i(0:ubound(p_i,1))/B_i(0:ubound(p_i,1))/U_i(0:ubound(p_i,1))) * (D0/(B0*u0))
        prec(i_sol)%u(0:ubound(p_i,1)) = U_i(0:ubound(p_i,1)) * u0
        prec(i_sol)%P(0:ubound(p_i,1)) = U_i(0:ubound(p_i,1))**(-Gth) &
                                       * (1+zeta*(Gth-1)*Ms0**2/Ma0 * (1-U_i(0:ubound(p_i,1))**Gth)) * P0
        prec(i_sol)%B(0:ubound(p_i,1)) = B_i(0:ubound(p_i,1)) * B0
...
!===========================================================================================
 subroutine solve_coupled_system(Gth, u0, P0, T0, Ms0, Ma0, zeta, Rprec, &
                                 pres, Emax, tmax, xmax, xi, pinj0, eta0, D0, alpha, B0, &
                                 Rsub, Rtot, P2, T2, Pw2, pinj_xi, eta_xi, p, f, U)
!===========================================================================================
...
!===========================================================================================
    ...
    if(pinj0>0)then
      ...
      if(pinj_xi>0)then
        ...
        call error("set_pinj", "injection momentum not defined: set xi or pinj0", .true.)
      endif
    endif
    ...
    if(eta0>0)then
      ...
      if(eta_xi>0)then
        ...
        call error("set_pinj", "injection fraction not defined: set xi or eta0", .true.)
      endif
    endif
...
!===========================================================================================
    ...
    sum(0) = 0.
    x_left = U(0)/(U(0)-1/Rtot)
    do i = 1,ubound(p,1)
        x_right = U(i)/(U(i)-1/Rtot)
...
!===========================================================================================
 subroutine accel_electrons(kappa, chi, Emax_e, alpha, pres, Rsub, n2_p, T2_p, lnp_p, lnf_p, &
                            p_e, f_e)
!===========================================================================================
...
!===========================================================================================
    ...
    if(alpha<0)then ! Bohm: D(p) = D0/B.p^2/sqrt(1+p^2)
        ...
        lnp_e_inj_p = log((me/mp)**alpha*pinj_p)
    endif
    i = n_e
