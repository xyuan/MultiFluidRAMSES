!===========================================================================================!
! Blasi's non-linear model for Diffusive Shock Acceleration               v 1.10 (29/07/10) !
!===========================================================================================!
! units:                                                                                    !
...
module blasi_module
    ...
    real*8,parameter::delta_min=1d-4    ! numerical tolerance for numerical checks: requested precision
...
!===========================================================================================
    ...
    if(verbose>=1)then
        write(*,*)
        write(*,*)'Blasi_DSA(Ms0 = ',Ms0,', u0 = ',u0/1d5,'km/s, T0 = ',T0,'K, n0 = ',n0,'cm-3',', B0 = ',B0,&
                  'G, zeta = ',zeta,', D0 = ',D0,'G.cm^2/s, alpha = ',alpha,', xi = ',xi,', pinj0= ',pinj0/mc,'mp.c, eta0 = ',eta0,&
                  ', Emax_p = ',Emax_p/mc2,'mp.c^2, tmax = ',tmax_p/year,'yr, xmax = ',xmax_p/parsec,'pc, cut_p = ',cut_p,&
...
!===========================================================================================
    ...
    do i_sol=1,n_sol
        ...
        enddo
        B_i(0:n_nt) = sqrt(1 + ((1-zeta)/(4*Ma0)) * U_i(0:n_nt)**(-3/2.)*(1-U_i(0:n_nt)**2) * 2*Ma0**2)
...
!===========================================================================================
    ...
    if(Pw1>0)then ! magnetized shock (Caprioli et al 2008, 2009)
...
!===========================================================================================
    ...
    if(xi0<=0)then
        ...
        if(pinj0<=0.and.eta0>0)then
            ! find xi such that eta(xi) = eta0 by dichotomy
            xi_left  = sqrt(3/2.)
            xi_right = 5.
            delta = 2*delta_max
            iter = 0
...
!===========================================================================================
    ...
    if(Emax<=0.and.tmax<=0.and.xmax<=0)then
        pmax = pinj
        return
    endif
    pmax = 0
