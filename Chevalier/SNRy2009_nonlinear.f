      subroutine acceleration(eta_in,rho_anne,radius_sk_pc,u_z_kmps,
     &snr_age,fraction_sk,en_max_ev_in,r_tot,Pgaz,Pcos,FE,PgazsurqVs2,
	...
	integer n_passage_blasi
      ...
      real*8 eta_in,rho_anne,radius_sk_pc,u_z_kmps,r_tot,Pcos_sur_Pgaz
	real*8 Pgaz,Pcos,FE,PgazsurqVs2,PcossurqVs2,diff_len
	
      real*8 bmag_in, gyro_in,temp_in, eta_init
	real*8 snr_age,fraction_sk
	
	COMMON /Bmagn/ bmag_in,gyro_in, temp_in, eta_init, n_passage_blasi
...
!      fraction_sk = 0.1
      ...
      call simple_sub(eta_in,rho_anne,radius_sk_pc,u_z_kmps, 
     &                temp_in,bmag_in, snr_age,fraction_sk,en_max_ev_in,
     &                elec_temp_fac,r_tot, pres_gas_2, pres_en_tot,
...
c      print*,'diff_len/radius  radius',diff_len,radius_sk_pc_in
      diff_len = diff_len*radius_sk_pc
...
c       if((i_read .eq. 1).AND.(i_anne .eq. 2)) write(6,5114)
 5114  format(' Enter DS B-field (G) (FS)',
     ...
     #  ' "-1" for B_2 = B_0*r_tot AND sin = 1/2')
       if(i_anne .ne. 1) then
...
c
c       if(i_anne .eq. 1) en_max_ev_in = 1.0d13            ! fixed_value
c passe dans l'appel
c
...
c
       if(i_anne .eq. 1) x_lambda_in = 3.5             ! fixed_value
c
c
...
c
     ...
     #      bmag_z_in, fraction_sk, en_max_ev_in,r_tot_snr, 
     #      p_max, p_elec_max_cgs,bmag_ds)
         diff_len_max_o_radius_FS = diff_len_max_o_radius
      endif
c
c
       call cal_comp_bere(den_pro_z_in, p_max, vel_FS_cgs, 
     #    x_lambda_in, eta_ups_in, r_sub, r_tot, 
...
** AD
      if(dabs((r_tot_snr - r_tot)/r_tot) .gt. 0.001d00) then
         r_tot_snr = r_tot
         goto 9133
      endif
c
...
c
         ...
         write(22,3753) i_vol
c
c         write(6,2203) r_sub, r_tot,
c     #      sonic_mach_z_in, alf_mach_z_in
         write(22,2203) r_sub, r_tot,
...
c
          call cal_comp_bere(den_pro_z_in, p_max, vel_FS_cgs, 
     #       x_lambda_in, eta_ups_temp, r_sub, r_tot, 
     #       p_inj, p_berez, a_inj, a_mc, a_max, 
     #       q_sub, q_int, q_min, pres_gas_z_in, 
     #       sonic_mach_z_in, alf_mach_z_in, p_inj_input_mc, 
     #       pres_en_tot, en_eff_norm, press_frac, en_eff_absolute, 
...
c
 2203  format(/,' FS:  r_sub =',1pe9.2,'   r_tot =',1pe9.2,
     #    '   M_S0 = ',1pe9.2,'  M_alf =',1pe9.2)
 2204  format(/,' RS:  r_sub =',1pe9.2,'   r_tot =',1pe9.2,
     #    '   M_S0 = ',1pe9.2,'  M_alf =',1pe9.2)
...
c     #                 pro_num_total, (den_pro_z_in*r_tot)
         write(22,5509) snr_dist_kpc,
     #                 pro_num_total, (den_pro_z_in*r_tot)
...
c
      ...
      if(i_vol .eq. 66) call volume_int(fraction_sk) 
c
c
      call spectrum(p_berez, p_max, p_inj,
     #   a_inj, a_mc, a_max,
...
c
      if(snr_emis_vol .gt. 0.0) then
         ...
         shell_thickness    = rad_FS_cgs/(3.0*r_tot)
...
c
     ...
     #    x_lambda_in, eta_ups, r_sub, r_tot, p_inj, p_berez,
...
c
     ...
     #           r_sub*(gam_z - 1.0)) )
c
      r_tot = r_sub * (sonic_mach_z/xmach_s1)**(1.0/sig)
c
      if(r_tot .lt. r_sub) then
             r_sub_start = r_sub_save
             r_sub_del   = r_sub_del * 0.3
             goto 1002
      endif
c
      pres_gas_1 = pres_z * (r_tot/r_sub)**gam_z
c
      r_sub_alf = r_sub
      r_tot_alf = r_tot
c
...
c
            r_tot      = r_tot_temp
            r_tot_alf  = r_tot *(1.0 - (1.0/alf_mach_z)) ! ????????????
            r_sub_alf  = r_sub - (r_tot/alf_mach_z)  ! ??????????????
      endif
c
c
...
ccc      ds_sound_speed = (u_z/r_tot) * dsqrt(gam_z*(r_sub - 1.0))
c
c           ! Below is more correct expression from Ferraro & Plumpton, eq. 4.28
      pg1   = pres_z*(r_tot/r_sub)**gam_z
      xm1sq = (sonic_mach_z**2)*((r_sub/r_tot)**(gam_z+1.0))
      pg2   = pg1*(2.0*gam_z*xm1sq - (gam_z - 1.0))/(gam_z + 1.0)
c
      ds_sound_speed = dsqrt(gam_z*pg2/(rho_z*r_tot))
c
c
...
c
	den_1   = den_pro_z_in * (r_tot/r_sub)  ! This remains just proton density
...
c                            ! electrons and helium
       pres_gas_2 = rho_z*r_tot*(ds_sound_speed**2)/gam_gas_2
c
...
c
       esc_en_prime = 1.0 +
     #                (2.0/((gam_0 - 1.0)*xms**2)) +
     #                (gam_2 + 1.0)/((gam_2 - 1.0)*r_tot**2) -
     #                ((2.0*gam_2)/((gam_2 - 1.0)*r_tot)) *
     #                (1.0 + 1.0/(gam_0*xms**2))
c
...
c *****************************************************************
c
      subroutine pmax_cal(snr_age_sec, rad_cgs, vel_cgs, bmag_z_in, 
     &  fraction_sk, en_max_ev_in, r_tot, p_max, p_elec_max_cgs, 
     & bmag_ds)
c
      implicit real*8 (a-h, o-z), integer(i-n)
c
...
c	p_max_energy = en_max_ev_in * evterg/ccgs
      xmc = 5.014394050221500E-014
	xmp = 1.672621580000000E-024
	c   = 29979250000.0000
	p_max_energy = xmc * sqrt((en_max_ev_in* evterg/(xmp*c**2))**2-1)

c      print*,'p_max_in cgs et en_max_ev_in/evterg ',p_max_energy,
c     & en_max_ev_in* evterg

c	print*,snr_age_sec/3e7,eta_mc,v_sk_SI/1e3,u_2_SI/1e3,b_z_SI*1e4,b_2_SI*1e4
...
c	    print*,'p_max/mc = min(',p_max_energy  /xmc,' [F],'
c     #                        ,pmax_age *1.D6/xmc,' [A],'
c     #                        ,pmax_diff*1.D6/xmc,' [S])'
c
...
c
     ...
     #                 v_sk_SI *
     #                 dsqrt(1.0 - (1.0/r_tot)) *
     #              ( (1.0/bmag_z_in) + (r_tot/bmag_ds))**(-0.5) *
...
c      print*,'D0 = ',1e4*diff_length_max_SI*v_sk_SI*b_z_SI*xmc/p_max_SI
c
      diff_len_max_o_radius = diff_length_max_SI/rad_SI
c      print*,'x_max/x_FS = ',diff_len_max_o_radius
c
      return
      end
c
...
c
      den_2      = den_pro * r_tot  ! DS proton number density only
c
      eff_den_ds = den_2 *
     #          ( 1.0d00 + 
     #            frac_helium*frac_helium_ds_temp +
...
c *****************************************************************
c
      subroutine volume_int(fraction_sk)
c
      implicit real*8 (a-h, o-z), integer(i-n)
c
      common /comm2/pres_tot_2, pres_rel_tot, 
...
c
     ...
     #          bmag_temp_ups, fraction_sk, en_max_ev_in, r_tot_snr, p_max, 
     #          p_elec_max_cgs,bmag_temp_ds)
c
c
...
c
c
             if(dabs((r_tot_snr - r_tot)/r_tot) .gt. 0.001) then
                r_tot_snr = r_tot
                goto 9233
             endif
           endif
...
c
c           write(6,2003) r_sub, r_tot, sonic_mach, alf_mach
...
c
c           write(6,2004) r_sub, r_tot, sonic_mach, alf_mach
c           write(6,2006) (vel_RS_cgs/1.0d05), (rad_RS_cgs/pc_to_cm)
        endif
c
 2003  format(/,/,' FS:  r_sub =',1pe9.2,'   r_tot =',1pe9.2,
     #    '   M_S0 = ',1pe9.2,'  M_alf =',1pe9.2)
 2004  format(/,' RS:  r_sub =',1pe9.2,'   r_tot =',1pe9.2,
     #    '   M_S0 = ',1pe9.2,'  M_alf =',1pe9.2)
...
c *****************************************************************
c
      subroutine write_gam(write_files,i_sp, r_tot)
c
      implicit real*8 (a-h, o-z), integer(i-n)
	logical write_files
c
...
c
      if(write_files) write(30,445) r_tot
 445  format(1pe11.3,5x,' comp. ratio (r_tot)')
c
...
c
      if(write_files) write(31,445) r_tot
c
      if(write_files) write(31,448) den_pro_z_RS
c
      if(bmag_ds_in_RS .lt. 0.0) then
         bmag_ds    = bmag_ups_RS*r_tot
         xsin_synch = 0.5d00
      else
...
c
      if(snr_emis_vol .gt. 0.0) then
         ...
         shell_thickness    = rad_RS_cgs/(3.0*r_tot)
         shell_thickness_pc = shell_thickness/pc_to_cm
