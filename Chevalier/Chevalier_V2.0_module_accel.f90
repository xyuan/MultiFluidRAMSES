...
!===========================================================================================
    ...
    IN%verbose = 0                                ! to write some debug info (levels 1,2,3,4)
    ...
    ! maximum energy
    if(abs(DSA(iSh)%Emax)>0)then
      IN%Emax_p = DSA(iSh)%Emax * mp*c**2         ! maximum energy of protons [proton rest mass -> cgs]
      IN%tmax_p = SN%t * yr                       ! acceleration time of protons [years -> seconds]
      IN%xmax_p = DSA(iSh)%frac * SNR(iSh)%r_Sh   ! maximum diffusion length of protons [cm]
    else
      IN%Emax_p = 0
      IN%tmax_p = 0
      IN%xmax_p = 0
