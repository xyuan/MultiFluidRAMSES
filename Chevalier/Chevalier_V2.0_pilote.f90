!===========================================================================================!
! Chevalier's model for young supernova remnants                                      v 2.0 !
!===========================================================================================!
! simple test program for the module                                                        !
!===========================================================================================!
! Gilles Ferrand (CEA/Irfu/SAp)                                                  26/08/2010 !
!===========================================================================================!
...
program Chevalier
    ...
    SN%M = 5.     ! ejecta mass [solar masses]
    SN%n = 7      ! ejecta index
    SN%s = 0      ! ambient medium index
    SN%q = 1.     ! normalization of ambient density [amu/cm^(3-s)]
    ! mass composition        H    He   C    N    O
    SN%comp(-1) = composition(0.7, 0.3, 0.0, 0.0, 0.0)  ! ejecta
    SN%comp(+1) = composition(0.7, 0.3, 0.0, 0.0, 0.0)  ! ambient medium
    
    ! set acceleration parameters
    
    DSA(-1:+1)%shock_conditions = 'calculated' ! 'prescribed'
    ...
    DSA(-1:+1)%B0   = 5d-6      ! upstream magnetic field [G]
    ...
    call write_data_old("hydro_cr_V2old.dat",1.5,100,20)
    call write_data_new("hydro_cr_V2new.dat",1.5,1e-4)
    ...
    !================================================================================================
        ...
        real*4::res       ! radial resolution [pc]
        ! locals
        ...
        write(30,'(2x,"r [pc]",21x,"d [amu/cm3]",15x,"u [km/s]",17x,"P [dynes/cm2]",14x,"f",24x,"tS [yr]",20x,"n_tS [amu/cm3.yr]")')
        do i=0,N
            r = (i/(1d0*N)) * r_max ! [cm]
            write(30,*)r/pc, &
                       SNR_density(r)          , &
    ...
    !================================================================================================
        ...
        write(30,23)
        do i = i2,1,-1
            write(30,22)I0(i),R0(i)/pc,u0(i)/1d6,d0(i),P0(i),R0(i)/SN%r_CD
        enddo
        close(unit = 30)
